export function MasterCard() {
  return (
    <svg width="56" height="32" viewBox="0 0 56 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="0.5" y="0.5" width="55" height="31" rx="5.5" fill="white" />
      <rect x="0.5" y="0.5" width="55" height="31" rx="5.5" stroke="#F4F4F4" />
      <circle cx="22" cy="16" r="9" fill="#E80B26" />
      <circle cx="34" cy="16" r="9" fill="#F59D31" />
      <path
        d="M28 22.7085C29.8413 21.0605 31 18.6658 31 16.0002C31 13.3347 29.8413 10.9399 28 9.29199C26.1587 10.9399 25 13.3347 25 16.0002C25 18.6658 26.1587 21.0605 28 22.7085Z"
        fill="#FC6020"
      />
    </svg>
  );
}
